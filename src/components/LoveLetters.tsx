...
export default function LoveLetters() {
  ...
  useEffect(() => {
    // Garantir que temos uma carta válida
    ...
    let index = 0;
    const content = letter.content.trim(); // Remove espaços em branco extras
    
    const timer = setInterval(() => {
      // Verificação adicional para evitar undefined
      if (index < content.length && content[index] !== undefined) {
  ...
  // Verificação de segurança para evitar erros se não houver carta
  if (!letter) {
    return null;
  }
